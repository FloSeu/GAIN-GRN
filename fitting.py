...
if True:
        ...
        #from cmd import cmd,lock,unlock
        from . import selector
        import os
        import pymol
        from pymol import cmd
        #from cmd import cmd,lock,unlock,Shortcut, \
...
SEE ALSO
    align, pair_fit, fit, rms, rms_cur, intra_rms, intra_rms_cur, super
                ...
                try:
                        ...
                        # call the C function
                        r = cmd.cealign( _self._COb, sel1, sel2, float(d0), float(d1), int(window), int(gap_max) )
    ...
    align, pair_fit, fit, rms, rms_cur, intra_rms, intra_rms_cur
        '''
                ...
                try:
                        ...
                        r = cmd.align(_self._COb,mobile,"("+target+")",float(cutoff),
...
SEE ALSO
    super, cealign, pair_fit, fit, rms, rms_cur, intra_rms, intra_rms_cur
                ...
                try:
                        _self.lock(_self)
                        r = cmd.align(_self._COb,mobile,"("+target+")",
...
SEE ALSO
    fit, rms, rms_cur, intra_rms, intra_rms_cur, pair_fit
                ...
                mix = int(mix)
                with _self.lockcm:
                    r = cmd.intrafit(_self._COb, selection,
...
SEE ALSO
    fit, rms, rms_cur, intra_fit, intra_rms_cur, pair_fit
                ...
                try:
                        _self.lock(_self)
                        r = cmd.intrafit(_self._COb,"("+str(selection)+")",int(state)-1,1,int(quiet),int(0))
                finally:
                        _self.unlock(r,_self)
                if not isinstance(r, list):
                        r = DEFAULT_ERROR
...
SEE ALSO
    fit, rms, rms_cur, intra_fit, intra_rms, pair_fit
                ...
                try:
                        _self.lock(_self)
                        r = cmd.intrafit(_self._COb,"("+str(selection)+")",int(state)-1,0,int(quiet),int(0))
                finally:
                        _self.unlock(r,_self)
                if not isinstance(r, list):
                    r = DEFAULT_ERROR
...
SEE ALSO
	align, super, pair_fit, rms, rms_cur, intra_fit, intra_rms, intra_rms_cur
            ...
            try:
                _self.lock(_self)
                r = cmd.fit(_self._COb,sele1,sele2,2,
                        int(mobile_state)-1,int(target_state)-1,
...
SEE ALSO
    fit, rms_cur, intra_fit, intra_rms, intra_rms_cur, pair_fit
            ...
            try:
                _self.lock(_self)
                r = cmd.fit(_self._COb,sele1,sele2,1,
                        int(mobile_state)-1,int(target_state)-1,
                        int(quiet),int(matchmaker),float(cutoff),
                        int(cycles),str(object))
            finally:
...
SEE ALSO
    fit, rms, intra_fit, intra_rms, intra_rms_cur, pair_fit
            ...
            try:
                _self.lock(_self)
                r = cmd.fit(_self._COb,sele1,sele2,0,
                        int(mobile_state)-1,int(target_state)-1,
                        int(quiet),int(matchmaker),float(cutoff),
                        int(cycles),str(object))
            finally:
...
SEE ALSO
    fit, rms, rms_cur, intra_fit, intra_rms, intra_rms_cur
            ...
            try:
                _self.lock(_self)
                r = cmd.fit_pairs(_self._COb,new_arg, quiet)
            finally:
                _self.unlock(r,_self)
            if r < -0.5:
                raise pymol.CmdException
