...
def construct_id_occupancy(gain_collection, intervals, center_dirs, length, plddt_dir, names, seqs):
    ...
    for k in range(length):
        identifier = names[k].split("-")[0]
        plddt_values = plddt_dir[identifier]
        gain_start = gain_collection.collection[k].start
        #print(identifier, gain_collection.collection[k].name, len(plddt_values))
    ...
    print(id_collection[0])
    print(plddt_collection[0])
    print(gain_collection.collection[0].name)
    # Here, parse through the id_dirs to count the occurrence of positions per SSE
    # Dictionary to map any label identifier to a respective position.
    id_map = {}
    i = 0
    ...
    label_plddts = compact_label_positions(id_collection, plddt_collection, segments, return_unique=True)
    label_seq = compact_label_positions(id_collection, seq_collection, segments, return_unique=True)
